...
public class SemTextFeatureManager_Discriminative extends FeatureManager {
	...
	private enum FEATURE_TYPE {
		emission, transition, pattern, neural, embedding
	};
	...
	public SemTextFeatureManager_Discriminative(GlobalNetworkParam param_g, HybridGrammar g, SemTextDataManager dm) {
		...
		if (NetworkConfig.USE_OUTPUT_EMBEDDING) {
			this._Semantic2Vec = new SemanticEmbedding(SHAPE_SIZE);
			this._Semantic2Vec.loadInstances();
		}
	...
	private FeatureArray extract_helper(Network network, SemanticUnit p_unit, SemanticUnit[] c_units,
		...
		if (pattern_parent.isw()) {
			...
			ArrayList<Integer> outputFeatures = new ArrayList<>();
			...
			for (int historySize = 0; historySize <= NetworkConfig._SEMANTIC_PARSING_NGRAM - 1; historySize++) {
				...
				int featureSize = 1 + 1 + prevWord;
				// int[] fs = new int[1+1+prevWord];
				// we'll provide space for neural features here
				if (NetworkConfig.USE_NEURAL_FEATURES) {
					// fs = new int[1+1+prevWord*2]; // this is the maximum
					// number, could be less
					featureSize += prevWord;
				}
				if (NetworkConfig.USE_OUTPUT_EMBEDDING) {
					outputFeatures.clear();
					outputString.clear();
					featureSize += (1 + prevWord) * this.SHAPE_SIZE;
				}

				int[] fs = new int[featureSize];
				...
				if (NetworkConfig.USE_OUTPUT_EMBEDDING) {
					for (int k = 0; k < this.SHAPE_SIZE; k++) {
						outputFeatures
								.add(this._param_g.toFeature(network, FEATURE_TYPE.embedding.name(), "" + k, input));
				...
				for (int w = 0; w < prevWord; w++) {
					...
					if (!NetworkConfig.USE_NEURAL_FEATURES || !NetworkConfig.REPLACE_ORIGINAL_EMISSION
						...
						if (NetworkConfig.USE_OUTPUT_EMBEDDING) {
							for (int k = 0; k < this.SHAPE_SIZE; k++) {
								outputFeatures.add(
										this._param_g.toFeature(network, FEATURE_TYPE.embedding.name(), "" + k, input));
								outputString.add(output);
							}
						}
					}
				...
				if (NetworkConfig.USE_OUTPUT_EMBEDDING) {
					int normalFeatureSize = t;
					fv = new double[fs.length];
					// System.out.println("t: "+ t);
					// System.out.println("fs size: "+ fs.length);
					for (int k = 0; k < outputFeatures.size(); k++) {
						// if(t<fs.length)
						fs[t++] = outputFeatures.get(k);

					}
					int dim = 0;
					for (int k = 0; k < t; k++) {
						if (k < normalFeatureSize) {
							fv[k] = 1.0;

						} else {
							String curOutput = outputString.get(k - normalFeatureSize);
							if (dim >= this.SHAPE_SIZE) {
								dim = 0;
							}
							fv[k] = this._Semantic2Vec.getEmbeddingValueForDim(curOutput, dim++);
							fv[k] = 0;
						}
					}
				}
			...
			for (int historySize = 0; historySize <= NetworkConfig._SEMANTIC_PARSING_NGRAM - 1; historySize++) {
				...
				if (NetworkConfig.USE_OUTPUT_EMBEDDING) {
					outputFeatures.clear();
					outputString.clear();
					featureSize += (prevWord * this.SHAPE_SIZE);
				}
				int[] fs = new int[featureSize];
				String output, input;
	...
	private String getWord(Sentence sent, int index, int offset) {
		...
		if (target == -1) {
			...
			if (sent.get(target).getName().equals("")) { // for multiple
															// whitespaces..
				return "<UNK>";
			}
